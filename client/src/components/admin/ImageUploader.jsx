      ...
      <div
        ...
        className={`relative border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-all ${
          dragActive
            ...
            : 'border-white/10 hover:border-[#D4AF37]/50 bg-black/20'
        } ${uploading ? 'opacity-60 pointer-events-none' : ''}`}
      >
        <div className="flex flex-col items-center justify-center space-y-2.5">
          {uploading ? (
            <Loader2 className="w-8 h-8 text-[#D4AF37] animate-spin mb-1" />
          ) : (
            <UploadCloud className="w-8 h-8 text-[#D4AF37] mb-1 group-hover:scale-110 transition-transform" />
          )}
          <span className="text-xs font-semibold text-white uppercase tracking-luxury">
            {uploading ? 'Uploading to Server...' : label}
          </span>
          <div className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg bg-[#D4AF37]/15 hover:bg-[#D4AF37]/25 text-[#D4AF37] border border-[#D4AF37]/40 text-xs font-semibold shadow-sm transition-all">

    ...
    const fetchCategoryAndProducts = async () => {
      try {
        ...
        if (catData.success && catData.category) {
          ...
          document.title = `${catData.category.name} | ${settings.companyName || 'NiceLamp'}`;
        }
        if (prodData.success) {
    ...
    return (
      <div className="min-h-screen pt-32 pb-20 bg-[#090a0d] flex items-center justify-center text-center px-4">
        <div className="max-w-md">
          <Layers className="w-12 h-12 text-[#D4AF37] mx-auto mb-4" />
    ...
    <div className="pt-24 pb-20 bg-[#090a0d] min-h-screen">
      ...
      <div className="relative border-b border-white/10 overflow-hidden bg-[#111318]">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <Link
            to="/catalog"
            className="inline-flex items-center gap-1.5 text-xs uppercase tracking-luxury text-[#D4AF37] hover:text-white transition-colors mb-6"
          >
          ...
          <div className="mt-6 flex items-center gap-3">
            <span className="text-xs font-mono text-[#D4AF37] bg-[#D4AF37]/10 border border-[#D4AF37]/20 px-3 py-1 rounded-md">
